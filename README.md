...
- `scripts/dev-admin/` — a dev-only manager at [http://localhost:5173/__admin](http://localhost:5173/__admin) for editing those lists (toggle experiments active/concluded, add/edit/reorder) and one-click publishing. Dev server only; never in a build. See [PROCESS.md](./PROCESS.md) §1c.
