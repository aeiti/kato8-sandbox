...
//
// Mirrors external-site/scripts/prerender.mjs. Sandbox-specific bits:
//   - SITE.url already includes the /kato8-sandbox subpath, so route
//     paths are appended as-is.
//   - Output paths are `/route` -> docs/route/index.html, matching how
//     GitHub Pages serves subdirectories under the base URL.
...
function buildSeoBlock(pathname, meta) {
  ...
  const lines = [
    ...
    meta.noindex && '    <meta data-prerender name="robots" content="noindex" />',
...
function main() {
  ...
  for (const pathname of routes) {
    ...
    if (!meta) throw new Error(`No SEO config for route ${pathname}`)
    ...
    console.log(`prerendered ${pathname.padEnd(36)} → ${rel}`)
  }
}
